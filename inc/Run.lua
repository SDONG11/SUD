...
ALI = Token:match("(%d+)")
redis:set(ALI..":VERSION",GetUser.information.Source_version)
redis:set(ALI..":SUDO_ID:",GetUser.information.id)
redis:set(ALI..":DataCenter:",GetUser.information.DataCenter)
redis:set(ALI..":UserNameBot:",BOT_User)
redis:set(ALI..":NameBot:",BOT_NAME)
redis:hset(ALI..'username:'..GetUser.information.id,'username','@'..GetUser.information.username:gsub('_',[[\_]]))
...
Bot_User = redis:get(ALI..":UserNameBot:")
SUDO_ID = tonumber(redis:get(ALI..":SUDO_ID:"))
SUDO_USER = redis:hgetall(ALI..'username:'..SUDO_ID).username
version = redis:get(ALI..":VERSION")
DataCenter = redis:get(ALI..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..ALI..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(ALI..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(ALI..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(ALI..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(ALI..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور 👨🏽‍💻'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(ALI..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء 👷🏽'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(ALI..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير 👨🏼‍⚕️' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(ALI..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن 👨🏼‍🎓'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(ALI..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
if msg.Admin then redis:srem(ALI..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(ALI..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(ALI..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.ALI then 
if msg.text and ScriptFile.iALI then
for k, ALI in pairs(ScriptFile.ALI) do
local SearchText = msg.text:match(ALI)
if SearchText then
if not CheckFlood(msg) then
return false 
end
local GetMsg = ScriptFile.iALI(msg,{msg.text:match(ALI)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",ALI.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dALI then
if ScriptFile.dALI(msg) == false then
return false
end
...
if Plug.ALI then 
if msg.text and Plug.iALI then
for k, ALI in pairs(Plug.ALI) do
local SearchText = msg.text:match(ALI)
if SearchText then
if not CheckFlood(msg) then
return false
end
local GetMsg = Plug.iALI(msg,{msg.text:match(ALI)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",ALI.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source ALI")
end 

end
else
print("The File Script.lua Not Runing in The Source ALI")
...
function tdcli_update_callback(data)
	...
	redis:srem(ALI..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/SDONG11/SUD/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/SDONG11/SUD/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/SDONG11/SUD/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/SDONG11/SUD/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/SDONG11/SUD/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/SDONG11/SUD/master/plugins/games.lua','./plugins/games.lua')
	...
	if redis:get(ALI..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(ALI..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(ALI..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(ALI..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(ALI..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(ALI..'lock_bots'..msg.chat_id_)
	ISBOT = false
	ZISBOT = false
	...
	if ZISBOT and redis:get(ALI..'lock_bots_by_kick'..msg.chat_id_) then
	...
	if redis:get(ALI..'group:add'..data.chat_.id_) then
	redis:set(ALI..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(ALI..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(ALI..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(ALI..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	ALI..'group:name'..k,IDS.Title,
	ALI..'num_msg_SUD'..k,5,
	ALI..'group:add'..k,true,
	ALI..'lock_link'..k,true,
	ALI..'lock_spam'..k,true,
	ALI..'lock_webpage'..k,true,
	ALI..'lock_markdown'..k,true,
	ALI..'lock_flood'..k,true,
	ALI..'lock_bots'..k,true,
	ALI..'mute_forward'..k,true,
	ALI..'mute_contact'..k,true,
	ALI..'mute_document'..k,true,
	ALI..'mute_inline'..k,true,
	ALI..'lock_username'..k,true,
	ALI..'replay'..k,true
	)
	redis:sadd(ALI..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(ALI..'username:'..ID,'username',user)
	redis:sadd(ALI..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(ALI..'username:'..ID,'username',user)
	redis:sadd(ALI..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(ALI..'username:'..ID,'username',user)
	redis:sadd(ALI..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(ALI..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(ALI..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(ALI..'username:'..data.user_.id_).username
	...
	redis:hset(ALI..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
